...
export default function Home() {
  ...
  return (
    <div className="flex min-h-screen w-full bg-slate-100/90 text-slate-900">
      {/* Sidebar */}
      <aside className="flex w-64 shrink-0 flex-col border-r border-slate-800/30 bg-gradient-to-b from-slate-900 via-blue-950 to-slate-950 text-slate-100 shadow-xl shadow-slate-900/20">
        ...
        <nav className="flex flex-1 flex-col gap-1 p-4">
          ...
          {[
            ...
            { label: "Thư viện bài dạy", active: false },
            { label: "Lịch dạy", active: false },
            { label: "Hồ sơ lớp", active: false },
          ].map((item) => (
            <button
              key={item.label}
              type="button"
              className={`rounded-lg px-3 py-2.5 text-left text-sm transition-colors ${
                item.active ? "bg-white/10 font-medium text-white ring-1 ring-white/10" : "text-slate-300 hover:bg-white/5 hover:text-white"
              }`}
            >
              {item.label}
            </button>
